...
import { cssSync, MOUNT_PREFIX } from "./index.js";
...
describe("cssSync", () => {
  ...
  it("threads an explicit root through to the source-locator", () => {
    ...
    const plugins = cssSync({ root: os.tmpdir() });
  ...
  it("mounts the apply engine on the dev server under the prefix", async () => {
    const plugins = cssSync({ root: os.tmpdir() });
    const enginePlugin = plugins.find((p) => p.name === "css-sync:engine");
  ...
  it("omits the engine plugin when engine:false", () => {
    const plugins = cssSync({ engine: false });
    expect(plugins.some((p) => p.name === "css-sync:engine")).toBe(false);
    expect(plugins.some((p) => p.name === "css-sync:config")).toBe(true);
    expect(plugins.some((p) => p.name === "css-sync:source-locator")).toBe(true);
  });
});
